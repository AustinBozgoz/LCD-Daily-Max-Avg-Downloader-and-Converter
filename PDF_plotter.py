...
def PDF_maker(root,Tau,months,years,TauName,miamiMax,cityName,stationID,folder,city=False,HI_overlay=False,record_city_logs=False):
    ...
    os.makedirs(folder+'PDF plots\\',exist_ok=True)
    if ('HI' in TauName) &(HI_overlay==True):#if HI graph and overlays are selected
        ...
        fig.savefig(folder+'PDF plots\\'+stationID+' '+TauName+' overlay '+monthNames+' '+yearNames)
    
    else:
        fig.savefig(folder+'PDF plots\\'+stationID+' '+TauName+' '+monthNames+' '+yearNames)
...
def PDF_plotter(root,months,years,city=False,record_city_logs=True,HI_overlay=True):
    ...
    if city!=False:#for a specific city
        file,cityDir,stationID,stopCity=PDF_progress(root,city=city)#pick out city info
        Tau=tau_loader(root+file,months,years)#read file and store tau columns in tuple
        for i in range(len(TauNames)):
            PDF_maker(root,Tau,months,years,TauNames[i],miamiMaxes[i],cityDir,stationID,folder=root+cityDir+'\\'+stationID+'\\',city=city,HI_overlay=HI_overlay,record_city_logs=record_city_logs)
