...
def main(cwd):
    ...
    while finish==0:
        ...
        if '1' in selection:
            ...
            while guard==0:
                ...
                if (selection=='y') or (selection=='Y'):
                    ...
                    while guard2==0:
                        guard1=0
                        while guard1==0:
                            print('\n \nEnter the number of the starting month (e.g. April would be 4)')
                            startMonth=input()
                            if (startMonth.isnumeric()==True): 
                                if (float(startMonth).is_integer()==True):
                                    if (int(startMonth)>0) and (int(startMonth)<13):guard1=1
                        ...
                        guard1=0
                        while guard1==0:
                            print('\n \nEnter the number of the ending month (e.g. April would be 4)')
                            endMonth=input()
                            if (endMonth.isnumeric()==True): 
                                if (float(endMonth).is_integer()==True):
                                    if (int(endMonth)>0) and (int(endMonth)<13):guard1=1
                        ...
                        if endMonth<startMonth:print('End month should be after start month')
                        else: guard2=1
                    guard=1
                    months=[int(startMonth),int(endMonth)]
            ...
            while guard==0:
                print('\n \n Please select a function')
                print('(1) Analyze One City')
                print('(2) Analyze all cities within \\NOAA_LCD_CSVs\\')
                print('Note City Logs and SQL databases will only be saved with function (2)')
                selection=input()
                ...
                if '1' in selection:
                    ...
                    PDF_plotter.PDF_plotter(cwd+'\\NOAA_LCD_CSVs\\',months=months,years=years,city=city,record_city_logs=False)
                    logging.info('Completed single city analysis for %s'%city)
                    guard=1
                    finish=1
                    ...
                    while guard1==0:
                        ...
                        if (selection=='y') or (selection=='Y'):
                            ...
                            while guard2==0:
                                ...
                                SQLtitle=input()
                                print('Is %s correct (y/n)?'%SQLtitle)
                                selection=input()
                                if (selection=='y') or (selection=='Y'):guard2=1
                            guard1=1
                        elif (selection=='n') or (selection=='N'): 
                            SQL=False
                            guard1=1
                        else: ('Please respond y or n')
                    ...
                    if SQL==True:
                        ...
                        sql_databaser.sql_databaser(cwd+'\\NOAA_LCD_CSVs\\',SQLtitle)
                        logging.info('Completed Analysis of all cities in \\NOAA_LCD_CSVs\\')
                    guard=1
                    finish=1
                else: print('Please make a selection 1-2 \n \n')
